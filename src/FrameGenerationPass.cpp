                                               ...
                                               std::function<void(const FrameGenerationProgress&)> onProgress)
{
    ...
    // Video only, for a caller that attaches the streams itself: the staging
    // encode already is the output, so it is renamed into place rather than
    // muxed with streams that would only be thrown away.
    if (!request.carryStreams) {
        std::error_code replaced;
        std::filesystem::remove(request.output, replaced);
        std::filesystem::rename(stagingGuard.path, request.output, replaced);
        if (replaced) {
            return fail(FrameGenerationError::Encoder,
                        L"The converted video could not be moved into place (error " +
                            std::to_wstring(replaced.value()) + L").");
        }
        ...
        LOG("Frame generation wrote " << std::dec << result.framesWritten << " frames ("
            ...
            << result.sourceFps << " -> " << result.outputFps << " fps, video only for the caller to mux");
        return result;
    }
