...
// carries a neutral value: zero motion, flat depth, empty mask. Ablations vary
...
struct GuideControls {
    ...
    constexpr bool IsDefault() const noexcept { return motionVectors && depth && mask; }
};

// Canonical form used for the worker argument, the cache identity term and
// log receipts: "mv=1,depth=1,mask=1". Parsing accepts only that exact shape.
...
inline std::optional<GuideControls> ParseGuideControls(std::string_view text)
{
    ...
    if (text.size() != mv.size() + 1 + depth.size() + 1 + mask.size() + 1) return std::nullopt;
    ...
    if (text.substr(offset, mask.size()) != mask) return std::nullopt;
    offset += mask.size();
    if (!flag(controls.mask)) return std::nullopt;
    return controls;
}
