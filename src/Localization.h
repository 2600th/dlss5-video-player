...
private:
    ...
    static const Map& EnglishDefaults() {
        ...
        {L"menu.dlss_upscaling", L"DLSS Upscaling"},
            ...
            {L"neural.settings.gpu_source", L"GPU source conversion"},
            ...
            {L"neural.tip.gpu_convert", L"Converts each rendered frame to NV12 on the GPU instead of letting the encoder convert on the CPU. Measured with the neural pass running, the GPU is the scarce resource and the CPU path was slightly faster, so this is off by default; turn it on for a GPU with headroom."},
            {L"neural.tip.gpu_source", L"Decodes the source to NV12 and converts it to BGRA on the GPU instead of letting ffmpeg convert on the CPU. Turn it off to compare when the GPU is the bottleneck. Applies to the next render."},
