...
private:
    ...
    static const Map& EnglishDefaults() {
        ...
        {L"menu.dlss_upscaling", L"DLSS Upscaling"},
            ...
            {L"menu.upscale_history_temporal", L"Temporal (steadier)\t49-80"},
            {L"menu.upscale_history_per_frame", L"Per-frame (sharper on some clips)\t63-94"},
            ...
            {L"export.stages.history_temporal", L"Temporal (steadier)"},
            {L"export.stages.history_per_frame", L"Per-frame (sharper on some clips)"},
            {L"export.tip.history", L"How Super Resolution uses earlier frames. Temporal, the default, accumulates them; Per-frame upscales every frame on its own. Measured 960x540 to 1080p on an RTX 4080 SUPER, VMAF against the 1080p original: Per-frame scored higher on 5 of 6 clips (+19 on a slow pan, +18 on lossy near-still footage) and 1.3 lower on a held frame. Temporal changes less from frame to frame than the source (up to 1.2 luma levels less), which calms grain and trails motion; Per-frame changes as much as the source does. Bicubic scored above both on every clip (70-96, against 63-94 for Per-frame and 49-80 for Temporal). The same choice is DLSS > Upscaling history for playback. With Neural rendering ticked the pass keeps Temporal, because the model runs on the same frames."},
