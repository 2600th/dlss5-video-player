...
private:
    ...
    void LoadVideoSettings(){
        ...
        m_gpuSourceConversion=GetPrivateProfileIntW(L"Encoding",L"GpuSourceConversion",1,SettingsPath().c_str())!=0;
    ...
    bool EnableUpscaling(uint32_t height){
        ...
        if(ready){
            ...
            m_upscalingError=outsideRange
                ?L"DLSS SR unavailable (source too small for a "+std::to_wstring(height)+L"p target)"
                :std::wstring(L"SR could not start; previous playback preserved");
    ...
    void StartNeuralJob(const std::wstring& mediaUrl,const std::wstring& audioUrl,const std::wstring& displayTitle,const std::wstring& pageUrl,MediaSourceKind sourceKind,YouTubeSourceQuality sourceQuality,const std::string& reuseSourceKey={},double expectedDurationSeconds=0.0,NeuralRenderRange range={},bool prepareOnly=false,NeuralJobKind kind=NeuralJobKind::Offline){
        ...
        try{
            HWND target=m_hwnd;const auto gpu=m_opt.detectedGpu.generation;const auto moduleDirectory=ExecutableDirectory();const auto cacheRoot=m_cacheRoot;const GuideControls guides=m_renderGuides;const NeuralSettings settings=m_neuralSettings;const HANDLE pauseEvent=m_neuralPauseEvent;const bool gpuColorConversion=m_gpuColorConversion;const bool gpuSourceConversion=m_gpuSourceConversion;const uint32_t nvencPreset=m_nvencPreset;
    ...
    void WatchNeuralJobProgress(){
        ...
        m_neuralProgressAt={};
        const bool live=m_liveSession;
        if(live)StopLiveNeuralSession(true);
        else{
            CancelNeuralJob(false);
    ...
    // Capture format for conversions: NV12 converted on the GPU, or BGRA converted by
    ...
    bool m_gpuSourceConversion=true;
